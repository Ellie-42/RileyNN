#ifndef RILEYNN_ESF_H
#define RILEYNN_ESF_H

#include <eigen/unsupported/Eigen/CXX11/Tensor>

////CREDIT TO STACKOVERFLOW USER DavidAce
////https://stackoverflow.com/a/48963679/19490448
////Edit 2
...
auto TensorMap(const Eigen::PlainObjectBase<Derived> &matrix, const Dims... dims) {
    return Eigen::TensorMap(matrix, std::array<long, static_cast<int>(sizeof...(Dims))>{dims...});
}
...
auto TensorMap(const Eigen::PlainObjectBase<Derived> &matrix) {
    if constexpr(Derived::ColsAtCompileTime == 1 or Derived::RowsAtCompileTime == 1) {
        return Eigen::TensorMap(matrix, matrix.size());
    } else {
        return Eigen::TensorMap(matrix, matrix.rows(), matrix.cols());
    }
}
